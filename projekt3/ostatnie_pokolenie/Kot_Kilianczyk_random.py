...
# Tu kiedyś, dawno temu, było podejście czysto probabilistyczne, i co getCheckFeedback były obliczane nowe prawdopodobieństwa.
# Ale zdaje się, że to podejście które liczy karty, a oj było sporo takich prób po drodze, jest nawet lepsze.
class Kot_Kilianczyk_random(Player):
    ...
    def putCard(self, declared_card):
        ...
        lower = []
        legit = []
        self.cards.sort(key=lambda x: x[0])  # sortuj, żeby znać najwięszką / najmniejszą
        # przy tym podziale sort to relikt przeszłości, ale może zostać, żeby mieć min na idx = 0
        if declared_card is not None:
