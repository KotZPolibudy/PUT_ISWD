...
class MyThirdPlayer(Player):
    ...
    def putCard(self, declared_card):
        ...
        if self.cheat:
            ...
            if declared_card[0] < self.cards[-1][0]:
                ...
                say = np.random.randint(0, len(known_cards), 1)  # losowa karta do oszustwa
                declaration = (known_cards[say[0]][0], known_cards[say[0]][1])
            else:
    ...
    def getCheckFeedback(self, checked, iChecked, iDrewCards, revealedCard, noTakenCards, log=True):
        ...
        # noTakenCards — użyć do pamiętania co jest na stosiku w nast wersji ;)
        if checked:
            if iChecked:
                # I Checked
                if iDrewCards:
            ...
            self.shouldCheat = min(0.5, self.shouldCheat + 2 * self.learningRate)
